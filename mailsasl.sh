#!/bin/sh
# Script to extract offending ip's from mail(log) which try to authenticate over SASL
...
# Filename of the resultfile
ResultFileName="sasl-offenders.txt"
...
# Remote webserver name for transferring it
RemoteServer="IP or FQDN"
...
# If it exists read the file for SASL login failures
    ...
    ipResults=$(cat $MailLogfile | grep -i sasl | awk '{print $5}'|grep -E -o "([0-9]{1,3}[\.]){3}[0-9]{1,3}"|sort --unique )
...
# send resultfile to webhost to be served for pickup by firewall   
        if [ $(($PostAddCount-$PreAddCount)) == 0 ]; then
            ...
            if [ $TransferResult == "yes" ]; then
                if [ -n $RemoteFileName ]; then
                    if [ $debug == "yes" ];  then
                        printf "Sending resultfile to remote as $ResultFilePath/$ResultFileName $RemoteUser"@"$RemoteServer":"$RemotePath"/"$RemoteFileName \n"
                    fi
                scp $ResultFilePath/$ResultFileName $RemoteUser"@"$RemoteServer":"$RemotePath"/"$RemoteFileName
                else
                    if [ $debug == "yes" ];  then
                        printf "Sending resultfile to remote as  $ResultFilePath/$ResultFileName $RemoteUser"@"$RemoteServer":"$RemotePath"/" \n"
                    fi
